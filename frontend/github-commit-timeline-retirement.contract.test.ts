...
const retired = "frontend/src/domains/github/presentation/components/github-commit-timeline.tsx";
const typePath = "frontend/src/domains/github/domain/types.ts";
const contract = "frontend/github-commit-timeline-retirement.contract.test.ts";
const forbidden = new Set(["GithubCommitTimeline", "GithubCommitTimelineProps"]);
const retiredToken = "github-commit-timeline";
...
const scanOne = (path: string, raw: string, allowRetiredType = false) => {
  ...
  const visit = (node: ts.Node): void => {
    if (allowRetiredType && path === typePath && ts.isInterfaceDeclaration(node) && node.name.text === "GithubCommitTimelineProps") return;
...
describe("GitHub commit timeline retirement boundary", () => {
  test("retires only the orphaned commit timeline and preserves the residual GitHub graph", () => {
    expect(retiredPresent()).toBe(false);
    expect(scan()).toBe(true);
    expect(read(typePath).includes("GithubCommitTimelineProps")).toBe(false);
  ...
  test("fails closed for relocated imports, symbols, and templates while ignoring prose", () => {
    ...
    const relocated = "frontend/src/domains/github/presentation/archive/github-commit-timeline.tsx";
    const mutations = [
      { [relocated]: "export const GithubCommitTimeline = () => null;" },
      { [importer]: append(valid, 'import { GithubCommitTimeline } from "../components/github-commit-timeline";') },
      { [importer]: append(valid, 'import type { GithubCommitTimelineProps } from "../../domain/types";') },
      { [importer]: append(valid, 'import {\n  GithubCommitTimeline,\n} from "../components/github-commit-timeline.tsx";') },
      { [importer]: append(valid, 'export { GithubCommitTimeline } from "../components/github-commit-timeline";') },
      { [importer]: append(valid, 'const load = () => import(`../components/github-commit-timeline.tsx`);') },
      { [importer]: append(valid, 'const load = require("../components/github-commit-timeline");') },
      { [importer]: append(valid, 'import Tab = require("../components/github-commit-timeline");') },
      { [relocated]: "export default () => null;" },
      { [importer]: append(valid, "const prose = `GithubCommitTimeline is retired`; // github-commit-timeline") },
      { [typePath]: append(read(typePath), "export const restoredCommitTimelineProps: GithubCommitTimelineProps | null = null;") },
