...
const retired = "frontend/src/domains/goals/presentation/components/milestone-progress-bar.tsx";
const typePath = "frontend/src/domains/goals/domain/types.ts";
const contract = "frontend/milestone-progress-bar-retirement.contract.test.ts";
const forbidden = new Set(["MilestoneProgressBar", "MilestoneProgressProps"]);
const retiredToken = "milestone-progress-bar";
...
const scanOne = (path: string, raw: string) => {
  ...
  const specs: string[] = [];
  let issue = posix.basename(path) === retiredBasename;
  const visit = (node: ts.Node): void => {
    if (path === typePath && ts.isInterfaceDeclaration(node) && node.name.text === "MilestoneProgressProps") return;
...
describe("milestone progress bar retirement boundary", () => {
  test("retires only the orphan progress bar and preserves goal/backend seams", () => {
    expect(retiredPresent()).toBe(false);
    expect(scan()).toBe(true);
    expect(read(typePath).includes("MilestoneProgressProps")).toBe(false);
  ...
  test("fails closed for relocated imports, symbols, and templates while ignoring prose", () => {
    ...
    const relocated = "frontend/src/domains/goals/presentation/archive/milestone-progress-bar.tsx";
    const mutations = [
      { [relocated]: "export const MilestoneProgressBar = () => null;" },
      { [importer]: append(valid, 'import { MilestoneProgressBar } from "./milestone-progress-bar";') },
      { [importer]: append(valid, 'import type { MilestoneProgressProps } from "@/domains/goals/domain/types";') },
      { [importer]: append(valid, 'import {\n  MilestoneProgressBar,\n} from "./milestone-progress-bar.tsx";') },
      { [importer]: append(valid, 'export { MilestoneProgressBar } from "./milestone-progress-bar";') },
      { [importer]: append(valid, 'const load = () => import(`./milestone-progress-bar.tsx`);') },
      { [importer]: append(valid, 'const load = require("./milestone-progress-bar");') },
      { [importer]: append(valid, 'import Progress = require("./milestone-progress-bar");') },
      { [importer]: append(valid, "const copy = `${MilestoneProgressProps}`;") },
      { [importer]: append(valid, "const prose = `MilestoneProgressBar is retired`; // MilestoneProgressProps") },
      { [typePath]: append(read(typePath), "export const restoredProgressProps: MilestoneProgressProps | null = null;") },
      { [retired]: "" },
    ];
