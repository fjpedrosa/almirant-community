...
import type { ProviderCardMinimalProps, ProviderType } from "../../domain/types";
...
// ---------------------------------------------------------------------------
// ProviderCardMinimal - Compact clickable card (icon + name + status dot)
// ---------------------------------------------------------------------------
// Usage:
//   <ProviderCardMinimal
//     provider="anthropic"
//     name="Anthropic"
//     status="connected"
//     onClick={() => openPanel("anthropic")}
//   />
// ---------------------------------------------------------------------------

export const ProviderCardMinimal: React.FC<ProviderCardMinimalProps> = ({
  provider,
  name,
  status,
  subtitle,
  ...
  return (
    <Card
      ...
      className={cn(
        ...
        comingSoon
          ...
          : "cursor-pointer hover:border-primary/40 hover:bg-accent/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
      )}
      aria-label={comingSoon ? `${name} - Coming Soon` : `${name} - ${status}`}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          ...
          {comingSoon && (
            <Badge
              variant="outline"
              className="shrink-0 border-blue-300 bg-blue-50 px-1.5 py-0 text-[10px] font-medium leading-4 text-blue-700 dark:border-blue-700/40 dark:bg-blue-900/30 dark:text-blue-400"
            >
              Coming Soon
            </Badge>
          )}
          {featureFlagged && !comingSoon && (
            <Badge
              variant="outline"
              className="shrink-0 border-amber-300 bg-amber-50 px-1.5 py-0 text-[10px] font-medium leading-4 text-amber-700 dark:border-amber-700/40 dark:bg-amber-900/30 dark:text-amber-400"
            >
              Flag
            </Badge>
          )}
      ...
      {/* Status dot - hidden for coming soon */}
      {!comingSoon && <ProviderStatusDot status={status} />}
    </Card>
  );
};
