...
import type { SeedTraceabilitySectionProps } from "../../domain/types";

export const SeedTraceabilitySection: React.FC<SeedTraceabilitySectionProps> = ({
  feedbackLinks,
  workItemLinks,
  isLoading,
}) => {
  const t = useTranslations("seeds.traceability");
    ...
    <Collapsible defaultOpen={hasLinks}>
      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm font-medium transition-colors hover:bg-muted/50 [&[data-state=open]>svg:last-child]:rotate-180">
        <div className="flex items-center gap-2">
          ...
          {t("title")} {totalLinks > 0 && `(${totalLinks})`}
        </div>
          ...
          {isLoading ? (
            ...
            <>
              {feedbackLinks.length > 0 && (
                <section className="space-y-2">
                  <h4 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    {t("feedback")} ({feedbackLinks.length})
                  </h4>
              ...
              {workItemLinks.length > 0 && (
                <section className="space-y-2">
                  <h4 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    {t("workItems")} ({workItemLinks.length})
                  </h4>
                  <div className="space-y-1.5">
