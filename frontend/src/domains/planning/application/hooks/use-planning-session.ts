...
import { useWsContextOptional, type WebSocketContextValue } from "@/domains/shared/application/hooks/use-ws-context";
...
type PlanningStartTransport = Pick<WebSocketContextValue, "isConnected" | "sendMessage">;
...
/**
 * Hand a `planning:start` to the WS transport.
 *
 * The transport queues-and-flushes on reconnect (see useWebSocket.sendMessage /
 * flushPendingMessages), so we must NOT drop the opening message of a session
 * when the socket is still mid-handshake — that was the
 * "startSession skipped: WS not connected" race that silently lost the first
 * message of a brand-new session while the title still got generated by a
 * separate REST call. We only bail when there is no transport context at all.
 * `onStreamingStart` runs (reset dedupe + START_STREAMING) only when we forward.
 * Returns true when the message was handed to the transport.
 */
export const startPlanningSessionSend = (
  wsContext: PlanningStartTransport | null | undefined,
  ...
  wsContext.sendMessage({
    ...
    payload: {
      ...
      ...(params.seedIds && params.seedIds.length > 0 ? { seedIds: params.seedIds } : {}),
      ...(params.agentConfig?.provider ? { provider: params.agentConfig.provider } : {}),
      ...(params.agentConfig?.codingAgent ? { codingAgent: params.agentConfig.codingAgent } : {}),
      ...(params.agentConfig?.model ? { model: params.agentConfig.model } : {}),
    },
  });
...
export const usePlanningSession = (): UsePlanningSessionReturn => {
  ...
  const startSession = useCallback(
    (sessionId: string, userMessage: string, seedIds?: string[], agentConfig?: { provider?: string; codingAgent?: string; model?: string }) => {
      if (!wsContext) {
        console.warn("[planning] startSession skipped: no WS context");
        return;
      }
      console.info("[planning] startSession: sending planning:start", {
        sessionId,
        agentConfig,
        isConnected: wsContext.isConnected,
      });

      // Do NOT gate on wsContext.isConnected: the transport queues-and-flushes on
      // reconnect, so gating here silently dropped the first message of a session
      // when the input was submitted before the WS finished its handshake.
      startPlanningSessionSend(
        wsContext,
        { sessionId, userMessage, seedIds, agentConfig },
        () => {
