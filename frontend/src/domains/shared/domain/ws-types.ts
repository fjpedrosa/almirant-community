// WebSocket protocol types - Client side
// Mirror of backend/api/src/ws/ws-types.ts
...
/**
 * Optional correlation field added to any client-initiated message.
 * Generated at the click site (crypto.randomUUID()) so the trace sink
 * can correlate ws-out entries with the ws-in frames of the resulting job.
 * The backend propagates clientActionId → traceId via AsyncLocalStorage.
 */
export interface WsClientActionMeta {
  clientActionId?: string;
}

export type WsClientMessage =
  | (WsClientPing & WsClientActionMeta)
  | (WsClientAiFormatRequest & WsClientActionMeta)
  | (WsClientPlanningStart & WsClientActionMeta)
  | (WsClientPlanningAnswer & WsClientActionMeta)
  | (WsClientPlanningPrompt & WsClientActionMeta)
  | (WsClientPlanningCancel & WsClientActionMeta)
  | (WsClientPlanningPrewarm & WsClientActionMeta)
  | (WsClientPlanningKill & WsClientActionMeta)
  | (WsClientPlanningInterrupt & WsClientActionMeta);
...
export interface WsServerPlanningResponseComplete {
  ...
  payload: {
    ...
    /** ISO timestamp when the interaction expires (for countdown timer). */
    expiresAt?: string | null;
  };
}
...
// --- Canonical event WS types (rich tool/file/bash/subagent events) ---
...
export interface WsServerPlanningMessageQueued {
...
export interface WsServerPlanningMessageDequeued {
