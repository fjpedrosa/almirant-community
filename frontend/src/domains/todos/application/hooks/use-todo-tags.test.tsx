...
let useAddTodoTag: typeof import("./use-todo-tags").useAddTodoTag;
...
beforeAll(async () => {
  ...
  ({ useAddTodoTag } = await import("./use-todo-tags"));
});
...
describe("useAddTodoTag tag-list invalidation", () => {
  it("invalidates the complete entity-then-global sequence only for a non-empty name", async () => {
    for (const testCase of cases) {
      ...
      const { result, unmount } = renderHook(() => useAddTodoTag(), { wrapper });
  ...
  it("waits for a successful response and does not invalidate on errors", async () => {
    ...
    const { client, wrapper } = harness(); const invalidate = spyOn(client, "invalidateQueries"); const { result, unmount } = renderHook(() => useAddTodoTag(), { wrapper });
    let mutation!: Promise<unknown>; await act(async () => { mutation = result.current.mutateAsync({ id: "todo-1", data: { name: "new" } }); await new Promise((resolve) => setTimeout(resolve, 0)); }); expect(invalidate).not.toHaveBeenCalled(); await act(async () => { resolveResponse(response()); await mutation; }); expect(keySequence(invalidate)).toEqual([["todos", "list"], ["todos", "detail", "todo-1"], ["tags", "list"]].map((key) => JSON.stringify(key))); invalidate.mockClear(); globalThis.fetch = (async () => response()) as unknown as typeof fetch; await act(async () => { await result.current.mutateAsync({ id: "todo-1", data: { tagId: "tag-2" } }); }); expect(keySequence(invalidate)).toEqual([["todos", "list"], ["todos", "detail", "todo-1"]].map((key) => JSON.stringify(key))); unmount(); client.clear();

    globalThis.fetch = (async () => errorResponse()) as unknown as typeof fetch;
    const failedHarness = harness(); const failedInvalidate = spyOn(failedHarness.client, "invalidateQueries"); const failed = renderHook(() => useAddTodoTag(), { wrapper: failedHarness.wrapper });
    await expect(failed.result.current.mutateAsync({ id: "todo-1", data: { name: "new" } })).rejects.toThrow(); expect(failedInvalidate).not.toHaveBeenCalled(); failed.unmount(); failedHarness.client.clear();
  });
});
