...
const retired = "frontend/src/domains/goals/presentation/components/milestone-metrics.tsx";
const typePath = "frontend/src/domains/goals/domain/types.ts";
const contract = "frontend/milestone-metrics-retirement.contract.test.ts";
const forbidden = new Set(["MilestoneMetrics", "GoalMetricsProps"]);
...
const scanOne = (path: string, raw: string) => {
  ...
  const visit = (node: ts.Node): void => {
    if (path === typePath && ts.isInterfaceDeclaration(node) && node.name.text === "GoalMetricsProps") return;
...
describe("milestone metrics retirement boundary", () => {
  test("retires only the orphan metrics view and preserves goal/backend seams", () => {
    expect(retiredPresent()).toBe(false);
    expect(scan()).toBe(true);
    expect(read(typePath).includes("GoalMetricsProps")).toBe(false);
  ...
  test("fails closed for relocated imports, symbols, and templates while ignoring prose", () => {
    ...
    const mutations = [
      { ["frontend/src/domains/goals/presentation/archive/milestone-metrics.tsx"]: "export const MilestoneMetrics = () => null;" },
      { [importer]: append(valid, 'import { MilestoneMetrics } from "./milestone-metrics";') },
      { [importer]: append(valid, 'import type { GoalMetricsProps } from "@/domains/goals/domain/types";') },
      { [importer]: append(valid, 'import {\n  MilestoneMetrics,\n} from "./milestone-metrics.tsx";') },
      { [importer]: append(valid, 'export { MilestoneMetrics } from "./milestone-metrics";') },
      { [importer]: append(valid, 'const load = () => import(`./milestone-metrics.tsx`);') },
      { [importer]: append(valid, 'const load = require("./milestone-metrics");') },
      { [importer]: append(valid, 'import Metrics = require("./milestone-metrics");') },
      { [importer]: append(valid, "const copy = `${GoalMetricsProps}`;") },
      { [importer]: append(valid, "const prose = `MilestoneMetrics is retired`; // GoalMetricsProps") },
      { [typePath]: append(read(typePath), "export const restoredMetricsProps: GoalMetricsProps | null = null;") },
      { [retired]: "" },
    ];
