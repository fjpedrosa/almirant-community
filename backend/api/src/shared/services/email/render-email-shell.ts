...
const DEFAULT_BODY_STYLE = "margin:0;padding:0;background:#f4f4f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;";

export interface EmailDocumentArgs {
  locale: string;
  title: string;
  body: string;
  headExtras?: string;
...
export const renderEmailDocument = (args: EmailDocumentArgs): string => `<!DOCTYPE html>
<html lang="${escapeHtml(args.locale)}">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>${escapeHtml(args.title)}</title>${args.headExtras ? `\n${args.headExtras}` : ""}
</head>
<body style="${args.bodyStyle ?? DEFAULT_BODY_STYLE}">
${args.body}
</body>
</html>`.trim();
...
export const renderEmailShell = (args: EmailShellArgs): string => renderEmailDocument({
  locale: args.locale,
  title: args.preheader,
  headExtras: `  <!--[if mso]>
  <style>body{font-family:Arial,sans-serif!important;}</style>
  <![endif]-->`,
  body: `  <!-- Preheader text (hidden) -->
