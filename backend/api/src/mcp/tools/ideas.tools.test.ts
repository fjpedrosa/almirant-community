...
mock.module("../setup", () => ({
  getOrganizationIdFromExtra: (extra: { authInfo?: { extra?: Record<string, unknown> } }) => {
...
describe("registerIdeasTools", () => {
  ...
  it("returns error when organization cannot be resolved", async () => {
    const tools = await buildToolsRegistry();
    const handler = tools.get("list_idea_items");
    ...
    expect(result.content[0]!.text).toContain("could not resolve organizationId");
  });
  ...
  it("returns mapped error when project does not belong to active organization", async () => {
    ...
    expect(result.isError).toBe(true);
    expect(result.content[0]!.text).toContain("does not belong to active organization");
