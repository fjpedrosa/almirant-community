...
const path = "backend/api/src/mcp/tools/ideas.tools.ts";
const names = "list_idea_items get_idea_item create_idea_item update_idea_item delete_idea_item set_idea_item_status assign_idea_item_owner set_idea_item_due_date toggle_idea_item_discussed promote_idea_item get_idea_item_traceability link_feedback_to_idea_item unlink_feedback_from_idea_item list_idea_comments add_idea_comment add_tag_to_idea_item remove_tag_from_idea_item list_idea_item_tags".split(" ");
...
describe("ideas-only assertOrgScope adoption", () => {
  test("covers the exact 18 handlers in manifest order", () => { expect(safe()).toBe(true); expect(records(read()).map(({ name }) => name)).toEqual(names); });
  test("fails closed for import, guard, callback, and manifest mutations", () => {
    ...
    const mutations = [
      ...
      valid.replace("  assertOrgScope,\n", "  assertOrgScope as canonicalAssertOrgScope,\n").replace("const IDEA_ITEM_TYPE_SCHEMA", "const assertOrgScope = () => \"fake\";\nconst IDEA_ITEM_TYPE_SCHEMA"),
      valid.replace("  assertOrgScope,\n", "").replace("const IDEA_ITEM_TYPE_SCHEMA", "const fakeImport = \"import { assertOrgScope } from '../setup'\";\nconst assertOrgScope = () => \"fake\";\nconst IDEA_ITEM_TYPE_SCHEMA"),
