...
const manifests = ([
  ["backend/api/src/mcp/tools/memory.tools.ts", "mem_save mem_search mem_context"],
  ["backend/api/src/mcp/tools/seed-memory.tools.ts", "seed_search seed_save"],
  ["backend/api/src/mcp/tools/todo-memory.tools.ts", "todo_search todo_save"],
  ["backend/api/src/mcp/tools/todos.tools.ts", "list_todo_items get_todo_item create_todo_item update_todo_item delete_todo_item set_todo_item_status assign_todo_item_owner set_todo_item_due_date list_todo_comments add_todo_comment"],
...
describe("memory/todo assertOrgScope adoption", () => {
  test("covers exact handlers in manifest order", () => { expect(manifests.every((entry) => safe(entry))).toBe(true); expect(manifests.flatMap((entry) => entry.names)).toHaveLength(17); });
  test("fails closed for import, callback, guard, and manifest mutations", () => {
    ...
    const replace = (needle: string, value: string) => { const next = valid.replace(needle, value); expect(next).not.toBe(valid); return next; };
    const mutations = [
      replace("  assertOrgScope,\n", "  assertOrgScope as canonicalAssertOrgScope,\n").replace("const workspaceId", "const assertOrgScope = () => \"fake\";\nconst workspaceId"),
      replace('from "../setup";', 'from "./scope-decoy" /* "../setup" */;'),
      replace("  assertOrgScope,\n", "").replace("import {", "// import { assertOrgScope } from '../setup'\nimport {").replace("const workspaceId", "const fake = `assertOrgScope`; const assertOrgScope = () => \"fake\";\nconst workspaceId"),
      replace('  assertOrgScope,\n', '  assertOrgScope,\n  assertOrgScope,\n'), replace(guard, "const scope = assertOrgScope(extra);"), replace(guard, `${guard}\n${guard}`),
      ...
      valid.replace('server.tool(\n    "mem_search"', 'server.tool(\n    "mem_save"'), valid.replace('server.tool(\n    "mem_save"', ""), valid.replace('server.tool(\n    "mem_save"', 'server.tool(\n    "renamed_memory_tool"'),
