...
import { assertOrgScope, getProjectIdFromExtra, getManagedByAgentFromExtra, getUserIdFromExtra, getPlanningSessionIdFromExtra, getPlanningMetadataFromExtra, getJobIdFromExtra } from "../setup";
...
export const registerWorkItemsTools = (server: McpServer) => {
  ...
  server.tool(
    ...
    "Record an AI session's token usage and cost for a work item. Call this when finishing work on a task to track AI resource consumption.",
    {
      ...
      model: z.string().describe("The AI model used (e.g. 'claude-opus-4-8', 'claude-sonnet-5')"),
  ...
  server.tool(
    ...
    "Atomically complete AI work on a task: move to Review, clear AI processing flags, set userActions, and record the AI session with token usage. Use this single call instead of separate move_work_item + update_work_item + record_ai_session calls.",
    {
      ...
      model: z.string().describe("The AI model that was used by the subagent (e.g. 'claude-opus-4-8', 'claude-sonnet-5')"),
      provider: z.string().optional().describe("AI provider (optional; inferred from model when omitted)"),
      ...
      aiModel: z.string().optional().describe("The AI model identifier to store on the work item column (e.g. 'claude-opus-4-8')"),
  ...
  server.tool(
    ...
    "Atomically complete validation for a task: move a passing task to its destination column (normally To Release), clear AI processing flags, merge documentation and test results metadata, and record the AI session. If a legacy Validating or To Document column ID is passed, the tool auto-redirects to To Release when that column exists on the board.",
    {
      ...
      model: z.string().describe("The AI model used (e.g. 'claude-opus-4-8')"),
      provider: z.string().optional().describe("AI provider (optional; inferred from model when omitted)"),
  ...
  server.tool(
    ...
    "Complete a failed validation for a task: move back to In Progress for corrective work, record diagnosis as structured event and visible comment, clear AI processing flags, and record the AI session. Legacy Needs Fix columns are supported only as a fallback; canonical boards use In Progress.",
    {
      ...
      model: z.string().describe("The AI model that performed the validation (e.g. 'claude-opus-4-8')"),
      provider: z.string().optional().describe("AI provider (optional; inferred from model when omitted)"),
      totalTokens: z.number().int().min(0).describe("Total tokens consumed (input + output)"),
      durationMs: z.number().int().min(0).optional().describe("Session duration in milliseconds"),
