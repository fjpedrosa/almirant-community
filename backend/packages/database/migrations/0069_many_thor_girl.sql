...
CREATE TABLE IF NOT EXISTS "idea_item_comments" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
...
DO $$ BEGIN ALTER TABLE "idea_item_comments" ADD CONSTRAINT "idea_item_comments_idea_item_id_idea_items_id_fk" FOREIGN KEY ("idea_item_id") REFERENCES "public"."idea_items"("id") ON DELETE cascade ON UPDATE no action; EXCEPTION WHEN duplicate_object THEN NULL; END $$;--> statement-breakpoint
DO $$ BEGIN ALTER TABLE "idea_item_comments" ADD CONSTRAINT "idea_item_comments_user_id_user_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."user"("id") ON DELETE no action ON UPDATE no action; EXCEPTION WHEN duplicate_object THEN NULL; END $$;--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "idea_item_comments_idea_item_id_idx" ON "idea_item_comments" USING btree ("idea_item_id");--> statement-breakpoint
...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'idea_item_comments'
  ) THEN
    INSERT INTO "entity_comments" ("id", "entity_type", "entity_id", "user_id", "content", "created_at", "updated_at")
    SELECT "id", 'idea', "idea_item_id", "user_id", "content", "created_at", "updated_at"
    FROM "idea_item_comments"
...
DO $$ BEGIN
  IF EXISTS (
    ...
    FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'idea_item_events'
  ) THEN
    INSERT INTO "entity_events" ("id", "entity_type", "entity_id", "event_type", "field_name", "old_value", "new_value", "triggered_by", "triggered_by_user_id", "metadata", "created_at")
    SELECT "id", 'idea', "idea_item_id", "event_type", "field_name", "old_value", "new_value", "triggered_by", "triggered_by_user_id", "metadata", "created_at"
    FROM "idea_item_events"
    ON CONFLICT ("id") DO NOTHING;
  END IF;
END $$;
