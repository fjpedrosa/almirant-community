 ...
 *   ORG_IDS=<uuid1>,<uuid2> bun run --env-file .env src/scripts/provision-auto-fix-bot-key.ts
 *
 * Re-execution:
...
import { createApiKey } from "../repositories/auth/api-key-repository";
...
if (orgIds.length === 0) {
  ...
  console.error(
    "   Example: ORG_IDS=abc123,def456 bun run src/scripts/provision-auto-fix-bot-key.ts"
  );
  process.exit(1);
}
...
// confirmed in backfill-organization.ts. We check manually per row.
